...
export default function Page() {
  ...
  useEffect(() => {
    ...
    addTelemetry('Obsidian CRM Telemetry initialized. Awaiting delivery webhooks.');
  }, [fetchCustomers, fetchSegments, fetchCampaigns, checkChannelServiceStatus, addTelemetry]);
  ...
  const handleRunCampaign = async (id: string) => {
    if (!channelOnline) {
      alert('Cannot launch campaign. The Channel Service is offline. Run `npm run channel` on port 3001.');
      return;
    }
  ...
  const handleSandboxLaunch = () => {
    ...
    setShowConsole(true);
    setActiveTab('segments');
    handleAiSegmentBuild();
  };

  if (!showConsole) {
    return (
      <LandingPage 
        onEnterConsole={() => setShowConsole(true)}
  ...
  const tabContent = {
    dashboard: {
      ...
      subtitle: 'Real-time delivery webhook feedback and revenue attribution metrics',
      component: (
        <TelemetryDashboard 
          campaigns={campaigns}
    ...
    segments: {
      ...
      subtitle: 'Target specific shopper groups visually or using AI natural language',
      component: (
        <SegmentBuilder 
    ...
    campaigns: {
      ...
      subtitle: 'Orchestrate multi-channel outreach and draft personalized copy with AI',
      component: (
        <CampaignComposer 
          step={composerStep}
    ...
    customers: {
      ...
      subtitle: 'Search and analyze individual customer records and behavioral history',
      component: (
        <CustomerDatabase 
          customers={customers}
    ...
    ingest: {
      ...
      subtitle: 'Batch import customer records or reset the environment with mock data',
      component: (
        <IngestPortal 
          json={ingestJSON}
    ...
    copilot: {
      ...
      subtitle: 'Autonomous assistant for strategy drafting and cohort discovery',
      component: (
        <AiCopilot 
